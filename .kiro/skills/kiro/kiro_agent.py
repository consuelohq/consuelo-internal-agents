#!/usr/bin/env python3
"""
Kiro Agent Orchestrator — spawn and manage Kiro coding sessions.

Uses `kiro-cli chat --trust-all-tools --no-interactive` for tool execution.
(ACP mode lacks --trust-all-tools, so we use chat mode as a workaround.)
...
OpenCode calls this to:
  ...
  2. Detect whether Kiro created specs or coded
...
def build_kiro_prompt(
    ...
    # permissions — folded into main prompt so we don't waste a turn on preamble
    parts.append("IMPORTANT: You have FULL permission to read files, write files, and run shell commands.")
    parts.append("Do NOT ask for confirmation. Work autonomously and complete the task fully.")
    parts.append("The ONE exception: NEVER delete files or directories.")
    parts.append("")
    ...
    parts.append("")
    parts.append("**CRITICAL:** You MUST use the github-dev CLI for ALL repo file operations.")
    parts.append("NEVER use your built-in file tools (readFile, writeTextFile, strReplace,")
    parts.append("editFile, etc.) for ANY repo files. Those tools modify the local filesystem,")
    parts.append("but this project is developed REMOTELY via the GitHub API. Built-in file tools")
    parts.append("will NOT affect the branch. Only use github-dev commands shown below.")
    parts.append("")
    ...
    parts.append("Use Python to write a temp file (avoids shell escaping issues), then stage it:")
    parts.append("```bash")
    parts.append("python3 -c \"")
    parts.append("content = '''")
    parts.append("... your code here ...")
    parts.append("'''")
    parts.append("with open('/tmp/kiro-edit.tmp', 'w') as f: f.write(content.lstrip('\\\\n'))")
    parts.append("\"")
    parts.append(f"python3 {GITHUB_DEV} write src/path/to/file.tsx --content-file /tmp/kiro-edit.tmp")
    parts.append("```")
    parts.append("")
    parts.append("**IMPORTANT:** Do NOT use `--content` flag with inline content (shell escaping breaks).")
    parts.append("Always write to a temp file first, then use `--content-file`.")
    parts.append("")
...
class KiroSession:
    """Manages a single Kiro coding session via chat mode."""
    ...
    def run_task(
        ...
        """
        Send a coding task to Kiro and wait for completion.
        ...
        self.on_progress("spawning kiro chat session...")
        self.on_progress(f"branch: {branch_name}")

        self.kiro_response = run_one_shot(
            prompt,
            cwd=self.cwd,
            timeout=self.timeout,
        )

        self.on_progress("kiro session complete, analyzing results...")
        result = self._analyze_session(self.kiro_response)
        return result

    def run_followup(self, prompt: str) -> str:
        """Send a follow-up prompt (spawns a new kiro session)."""
        self.on_progress("sending follow-up to kiro...")
        return run_one_shot(prompt, cwd=self.cwd, timeout=self.timeout)

    def _analyze_session(self, response: str) -> Dict[str, Any]:
        """Analyze Kiro's output to determine what happened."""
        response_lower = response.lower()

        # check if Kiro created specs
        spec_indicators = [
            "created linear issue", "created spec", "created issue",
            "linear_helper", "created the following specs",
            "created the following issues", "created tasks",
        ]
        ...
        commit_patterns = [
            r"committed", r"commit ", r"github-dev.*commit",
            r"files changed", r"wrote.*file",
        ]
        did_code = any(re.search(p, response_lower) for p in commit_patterns)

        # extract commit messages
        for match in re.finditer(r'commit\s+"([^"]+)"', response):
            self.commits_made.append(match.group(1))

        # extract file paths from github-dev write commands
        for match in re.finditer(r'write\s+([\w/.-]+)', response):
            self.files_changed.append(match.group(1))
