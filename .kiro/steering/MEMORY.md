...
- `promptSubmit` hooks: 5 individual hooks, one per file (SOUL.md, USER.md, MEMORY.md, ko.md, AGENTS.md). Use `runCommand` with `cat`. Output was getting truncated, so added `#[[file:]]` references in steering file as primary injection method (Feb 7 2026). Hooks kept as backup. Ko insists on `runCommand` — don't switch to `askAgent` without being asked.
- `agentStop` hook: auto-updates memory files after each conversation — updated Feb 7 2026 to write to `.kiro/steering/MEMORY.md` (the actual steering file) instead of root `MEMORY.md`
